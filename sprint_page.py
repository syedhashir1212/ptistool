...
def fetch_data_from_db(search_by=None, search_value=None, after_date=None, show_all=False):
    ...
    if show_all:
        ...
        SELECT * FROM inspections
        WHERE {search_by} > ?
        AND Customer IN ('SO&GSF', 'SO&GS', 'SOAGS', 'SOAGSF')
...
# # Fetch data from the database and standardize 'expire_date' column
# def fetch_data_from_db(search_by=None, search_value=None, after_date=None, show_all=False):
#     conn = sqlite3.connect('inspections.db')
...
#         params = ()
#     elif search_by == 'expire_date' and after_date:
#         # Convert after_date to string in 'YYYY-MM-DD' format
#         after_date_str = after_date.strftime("%Y-%m-%d")
#         query = f"""
#         SELECT * FROM inspections
#         WHERE DATE({search_by}) > ?
#         AND Customer IN ('SO&GSF', 'SO&GS', 'SOAGS', 'SOAGSF')
#         """
#         params = (after_date_str,)
...
#     # Standardize 'expire_date' column if it exists in the DataFrame
#     if 'expire_date' in df.columns:
#         df['expire_date'] = df['expire_date'].apply(standardize_date)
    
#     return df


# # Function to create a download link for PDF files
# def create_download_link(file_data, file_name):
#     b64 = base64.b64encode(file_data).decode()
#     href = f'<a href="data:application/octet-stream;base64,{b64}" download="{file_name}">Download PDF</a>'
#     return href
...
#     if search_by == 'expire_date':
#         after_date = st.date_input("Select a date to find records expiring after", value=datetime.today().date())
#         search_value = None
#     elif search_by == 'Show All Data':
#         after_date = None
#         search_value = None
